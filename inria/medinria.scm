...
(define-public vtk-8
  (package
    ...
    (source (origin
              ...
              (patches
               (search-patches "inria/patches/qpainterpath.patch"))
              (sha256
               (base32
     ...
     (list #:build-type "Release"          ;Build without '-g' to save space.
           #:phases #~(modify-phases %standard-phases
                                  (add-after 'unpack 'add-header
                                   (lambda _ (install-file "IO/Legacy/vtkLegacyReaderVersion.h" (string-append #$output "/include/vtk-8.1")))))
           #:configure-flags
    ...
    (source
     (origin
       ...
       (uri (string-append "https://github.com/InsightSoftwareConsortium/ITK/"
                           ...
                           version ".tar.gz"))
       (patches
        (search-patches "inria/patches/itkgdcmlimits.patch"))
       (sha256
        (base32 "13mmxm6m3xzqz32bbd7zh933ff5y3fr3b2l752vn2hq580wadqir"))))
    (arguments
     (list #:configure-flags #~'("-DITK_BUILD_DEFAULT_MODULES=ON"
                                 "-DBUILD_SHARED_LIBS=ON"
...
(define-public dcmtk-medinria-config
  (package
    ...
    (name "dcmtk-medinria")
    (arguments
     (list #:configure-flags #~'("-DBUILD_SHARED_LIBS:BOOL=ON"    
...
(define-public ttk
  (package
    ...
    (inputs
     (list vtk-8 insight-toolkit-5.1.1 hdf5-1.8 zlib libdrm mesa))
...
(define-public qtdcm
  (package
    ...
    (inputs
     (list insight-toolkit-5.1.1 dcmtk-medinria-config qtbase-5 qttools-5 qtx11extras zlib))
    (build-system cmake-build-system) 
    (arguments (list #:tests? #f     
                     #:build-type "Release"
...
(define-public medinria
  (package
    ...
    (inputs (list 
	      ...
	      vtk-8 
	      qtbase-5 qttools-5 qtx11extras qtdeclarative-5 qtsvg-5
	      rpi
	      ttk
	      boost))
    (build-system cmake-build-system) 
