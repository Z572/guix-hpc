...
node supercomputer parallel computations.")
   ...
   (source
    (origin
     ...
     (uri (git-reference
           (url home-page)
           (commit "fde4c96629c07ecdcb3a5b523cdac0588c7b287c")
...
(define-public composyx-librsb
  (package/inherit composyx
		   ...
		   (arguments (substitute-keyword-arguments (package-arguments composyx)
							    ((#:configure-flags flags
							      '())
							     `(cons "-DCOMPOSYX_USE_RSB=ON"
								    (cons "-DCOMPOSYX_USE_RSB_SPBLAS=ON"
									  ,flags)))))
		   (inputs `(("librsb" ,librsb)
			     ,@(package-inputs composyx)))))
