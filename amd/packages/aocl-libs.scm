        ...
        (source
            (origin
                ...
                (uri (git-reference
                    ...
                    (commit version)))
                (file-name (git-file-name name version))
                (sha256 (base32 "0k8fc3l5as2ga6b32d9zc0wqx8n11s7rvdi8qhsip1vbpkg8lpsm"))))
        (build-system cmake-build-system)
        (arguments (list #:tests? #f))
        (synopsis "The library AOCL-Utils is intended to provide a uniform interface to all
AOCL libraries to access CPU features, especially for AMD CPUs.")
        (description "AOCL-Utils is designed to be integrated into other AOCL libraries. Each
        ...
        (source
            (origin
                ...
                (uri (git-reference
                    ...
                    (commit version)))
                (file-name (git-file-name name version))
                (sha256 (base32 "12n2f1xc817f8bnmvijd0as1iggmjiz73vmyppszk7l0hgb3b6a9"))))
        (build-system gnu-build-system)
        (arguments
            (list
                #:tests? #f
                #:configure-flags
                #~(list "--enable-amd-flags")
                #:phases
                #~(modify-phases %standard-phases
                    (add-after 'unpack 'patch-config
                        (lambda _
                            (substitute* "build/config.mk.in" (("/usr/bin/env bash") (which "bash")))
                            (substitute* "src/aocl_dtl/Makefile" ((".PHONY") "CC=gcc\n.PHONY")))))))
        ...
        (source
            (origin 
                ...
                (uri (git-reference
                    ...
                    (commit version)))
                (file-name (git-file-name name version))
                (sha256 (base32 "0cz627x920ximydcs9qz6jilzia735hyg2flcd1lliq2xwvk1zd5"))))
        (build-system cmake-build-system)
        (arguments
            (list
                #:tests? #f
                ...
                #~(list
                    ...
                    (string-append "-DLAPACK_LIBRARIES=-lstdc++ " #$aocl-utils "/lib/libaoclutils.a " #$aocl-lapack "/lib/libflame.a")
                    (string-append "-DBLAS_LIBRARIES=-fopenmp " #$aocl-blis "/lib/libblis.a"))))
