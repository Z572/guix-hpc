...
(define-public npb-openmp
  (package
     ...
     (list
      ...
      #:phases #~(modify-phases %standard-phases
                   ...
                   (add-after 'unpack 'chdir
                     (lambda* _
                       (let* ((version' (string-take #$npb-version 3))
    ...
    (synopsis (string-append npb-synopsis ", OpenMP variant"))
    (description
...
(define-public npb-mpi
  (package/inherit npb-openmp
    (name "npb-mpi")
    (inputs (list openmpi))
    (arguments (list #:make-flags #~(list "suite")
                     #:tests? #f ;NPB Doesn't have tests
                     #:phases #~(modify-phases %standard-phases
                                  (delete 'install) ;NPB builds directly into the install dir
                                  
                                  (add-after 'unpack 'chdir
                                    (lambda* _
                                      (let* ((version' (string-take #$npb-version
                                                                    3))
                                             (subdir (string-append "NPB"
                                                                    version'
                                                                    "-MPI")))
                                        (chdir (pk 'subdir subdir)))))
                                  ...
                                  (replace 'configure
                                    (lambda* _
                                      ...
                                      (mkdir-p (string-append #$output "/bin")))))))
    (synopsis (string-append npb-synopsis ", MPI variant"))))
